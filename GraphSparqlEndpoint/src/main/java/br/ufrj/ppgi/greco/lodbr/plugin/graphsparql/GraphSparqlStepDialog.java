 ...
 * 
 */
public class GraphSparqlStepDialog extends BaseStepDialog implements
        StepDialogInterface
{
    ...
    public GraphSparqlStepDialog(Shell parent, Object stepMeta, TransMeta transMeta,
            String stepname)
    {
            ...
            ModifyListener defModListener)
    {
        ...
        wEndpointUri = swthlp.appendTextVarRow(cpt, null, "Endereço (URL):",
                defModListener);
        wDefaultGraph = swthlp.appendTextVarRow(cpt, wEndpointUri,
                "Grafo padrão:", defModListener);
        item.setControl(cpt);
        ...
        wPrefixes = swthlp.appendTableView(cpt, null, columns, defModListener,
                90);
        ...
        Composite ctl = swthlp.appendButtonsRow(cpt, wQueryString,
                new String[] { "Validar" }, listeners);

        wParserMessage = swthlp.appendMultiTextVarRow(cpt, ctl, defModListener,
                100);
    ...
    private void validateButton()
    {
        try
        {
            String fullQueryStr = GraphSparqlStepUtils.toFullQueryString(
                    this.getListOfPrefixesFromTableView(),
                    wQueryString.getText());

            // update message
            wParserMessage
                    .setText(GraphSparqlStepUtils.validateSparql(fullQueryStr));
        }
        catch (Throwable e)
        {
